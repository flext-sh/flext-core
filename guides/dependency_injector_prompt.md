...
- L1.5 (service runtime bootstrap): `u.create_service_runtime` (inherited by `s`) materializes settings/context/container in one call with optional overrides, registrations, and wiring. Prefer overriding `s._runtime_bootstrap_options` to pass parameters instead of duplicating setup logic.
...
- Service classes should reuse the runtime automation by overriding `_runtime_bootstrap_options` on `s` to feed parameters into `u.create_service_runtime` (settings overrides, service/factory/resource seeding, wiring targets). Avoid duplicating container/context/settings creation in constructors.
