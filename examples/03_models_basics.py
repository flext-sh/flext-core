...
def demonstrate_advanced_pydantic_mixins() -> None:
    ...
    class AdvancedEntity(F.IdentifiableMixin, F.TimestampableMixin, F.VersionableMixin):
    ...
    try:
        _invalid_entity = AdvancedEntity(name="ACTIVE_Invalid", is_deleted=True)
    ...
    try:
        ...
        invalid_timestamp.updated_at = invalid_timestamp.created_at.replace(second=0)
    except ValueError as e:
