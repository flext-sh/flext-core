...
def _check(label: str, value: _ContainerValue) -> None:
...
def _ser(v: _ContainerValue) -> str:
    if v is None:
        return "None"
...
class _ContainerStub:
    def __init__(self) -> None:
        self._services: dict[str, _ContainerValue] = {}
        self._config: dict[str, _ContainerValue] = {}
...
def demo_core_context_methods() -> None:
    ...
    _check("create.operation", ctx.get(c.Context.KEY_OPERATION_ID).unwrap_or("missing"))
    _check("create.user", ctx.get(c.Context.KEY_USER_ID).unwrap_or("missing"))
    _check("create.meta", ctx.get("meta_from_create").unwrap_or("missing"))
