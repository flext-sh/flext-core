...
- flext-core, flext-ldap, flext-ldif, flext-cli, client-a-oud-mig.
"""
...
class LDAPConnectionConfig(BaseModel):
    ...
    def validate_bind_dn(cls, v: str | None) -> str | None:
        ...
        if v is None:
            return v
        if "=" not in v:
            msg = "DN must contain attribute=value pairs (e.g., cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com)"
            raise ValueError(msg)
        return v
