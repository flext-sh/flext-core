...
class Ex03LoggingDsl(ExamplesFlextShared):
    ...
    def _exercise_factory_methods(self) -> None:
        ...
        _logger = u.create_module_logger("examples.ex_03.factory")
        self.audit_check("create_module_logger.protocol", True)
        _raw = u.fetch_logger("examples.ex_03.factory.raw")
        self.audit_check("fetch_logger.protocol", True)
        _wrapped = u.create_module_logger("examples.ex_03.factory.bound")
        self.audit_check("create_bound_logger.protocol", True)
    ...
    def _exercise_global_context(self) -> None:
        ...
        self.audit_check("global.info.ok", self._logged(logger.info("global bound")))
        self.audit_check(
    ...
    def _exercise_instance_methods(self) -> None:
        ...
        self.audit_check("bind.protocol", True)
        renewed = bound.new(stage="new")
        self.audit_check("new.protocol", True)
        unbound = renewed.unbind("stage")
        self.audit_check("unbind.protocol", True)
        safe = unbound.unbind("missing", "component", safe=True)
        self.audit_check("unbind_safe.protocol", True)
        self.audit_check("with_result.protocol", True)
        self.audit_check(
            "trace.ok", self._logged(safe.trace("trace value=%s", 1, key="t"))
        )
        self.audit_check(
            "debug.ok", self._logged(safe.debug("debug value=%s", 2, key="d"))
        )
        self.audit_check(
            "info.ok", self._logged(safe.info("info value=%s", 3, key="i"))
        )
        self.audit_check(
            "warning.ok",
        ...
        self.audit_check(
            "error.ok", self._logged(safe.error("error value=%s", 6, key="e"))
        )
        self.audit_check(
            "critical.ok",
            self._logged(safe.error("critical value=%s", 7, key="c")),
        )
        self.audit_check("log.ok", self._logged(safe.info("log value=%s", 8, key="l")))
        try:
            ...
            self.audit_check("build_exception_context.type", type(context_map).__name__)
            self.audit_check(
                "exception.ok",
                self._logged(
                    safe.exception(
    ...
    def _exercise_result_adapter(self) -> None:
        ...
        self.audit_check("adapter.name", logger.name)
        _rebound = logger.bind(adapter_key="v")
        self.audit_check("adapter.bind.protocol", True)
        self.audit_check(
            "adapter.trace.ok", self._logged(logger.trace("adapter trace"))
        )
        self.audit_check(
            "adapter.debug.ok", self._logged(logger.debug("adapter debug"))
        )
        self.audit_check("adapter.info.ok", self._logged(logger.info("adapter info")))
        self.audit_check(
            "adapter.warning.ok",
            self._logged(logger.warning("adapter warning")),
        )
        self.audit_check(
            "adapter.error.ok", self._logged(logger.error("adapter error"))
        )
        self.audit_check(
            "adapter.critical.ok",
