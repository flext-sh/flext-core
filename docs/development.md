...
3. **Configuration Alignment** – bootstrap logic pulls settings from `FlextConfig` and registers them in the container. Remember that processing helpers now rely on the `max_batch_size` and `max_handlers` fields, so tune those values through configuration rather than hard-coding limits.
