...
2. **Dispatcher coverage**: The guide still references the old bus era in a few narrative sentences. Update wording to point to `FlextDispatcher` for CQRS orchestration and remove `FlextBus` mentions entirely.
3. **Verification snippet**: The quick verification uses `python -c "from flext_core import __version__ ..."` which matches the current package surface.
4. **Examples**: All code samples compile against the current API after the import reduction. No runtime changes needed.
