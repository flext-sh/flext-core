...
class CreateUserCommand(FlextCommands.Models.Command):
    ...
    def _validate_role(self) -> FlextResult[None]:
        valid_roles = {"user", "REDACTED_LDAP_BIND_PASSWORD", "manager", "viewer"}
...
# Usage in endpoints
@router.post("/REDACTED_LDAP_BIND_PASSWORD/users", response_model=CreateUserResponse)
@require_auth(permission="users.create")
async def create_user_REDACTED_LDAP_BIND_PASSWORD(request: CreateUserRequest):
    """Create user (REDACTED_LDAP_BIND_PASSWORD only)."""
