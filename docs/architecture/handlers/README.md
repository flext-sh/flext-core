...
def role_based_authorizer(data: dict) -> bool:
    ...
    user_roles = {
        "REDACTED_LDAP_BIND_PASSWORD_001": ["REDACTED_LDAP_BIND_PASSWORD", "user"],
    ...
    if operation.startswith("REDACTED_LDAP_BIND_PASSWORD_"):
        return "REDACTED_LDAP_BIND_PASSWORD" in user_role_list
    elif operation.startswith("manager_"):
        return "manager" in user_role_list or "REDACTED_LDAP_BIND_PASSWORD" in user_role_list
...
auth_test_cases = [
    # Admin operation - should succeed for REDACTED_LDAP_BIND_PASSWORD
    {"user_id": "REDACTED_LDAP_BIND_PASSWORD_001", "operation": "REDACTED_LDAP_BIND_PASSWORD_delete_user", "target": "employee_003"},
    # Manager operation - should succeed for manager and REDACTED_LDAP_BIND_PASSWORD
    ...
    {"user_id": "employee_003", "operation": "REDACTED_LDAP_BIND_PASSWORD_delete_user", "target": "someone"},
...
class BusinessRulesHandler:
    ...
    def handle(self, request: dict) -> FlextResult[dict]:
        ...
        if operation == "delete" and user_role not in ["REDACTED_LDAP_BIND_PASSWORD", "manager"]:
            self.rule_violations += 1
...
test_requests = [
    ...
    # Business rule violation - should be blocked by business handler
    {
        ...
        "user_status": "active",
        "operation": "delete",  # Requires REDACTED_LDAP_BIND_PASSWORD/manager role
        "name": "Charlie User"
    },
