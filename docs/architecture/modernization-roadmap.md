...
2. **Validação Pydantic v2** consolidada (adapters reutilizáveis + menos validators duplicados).
3. **Módulos críticos com menor complexidade** (`runtime`, `checker`, `handlers`, `container`, `_models/*`).
4. **Testes de regressão** cobrindo casos de compatibilidade e serialização.
