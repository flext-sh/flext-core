...
from flext_core.constants import ConfigKey
from flext_core.constants import EntityId
...
from flext_core.constants import ServiceName
from flext_core.constants import TraceId
...
class TestAdvancedTypeAliases:
    """Unit tests for advanced type aliases."""

    def test_service_name_type_alias(self) -> None:
        """Test ServiceName type alias creation."""
        service_name = ServiceName("test-service")
        assert isinstance(service_name, str)
        assert service_name == "test-service"

    def test_config_key_type_alias(self) -> None:
        """Test ConfigKey type alias creation."""
        config_key = ConfigKey("app.config.key")
        assert isinstance(config_key, str)
        assert config_key == "app.config.key"

    def test_entity_id_type_alias(self) -> None:
        """Test EntityId type alias creation."""
        entity_id = EntityId("entity-123")
        assert isinstance(entity_id, str)
        assert entity_id == "entity-123"

    def test_trace_id_type_alias(self) -> None:
        """Test TraceId type alias creation."""
        trace_id = TraceId("trace-abc-123")
    ...
    def test_type_aliases_are_strings(
        self,
        type_alias: Callable[[str], str],
        value: str,
    ) -> None:
        """Test all type aliases create string instances."""
        instance = type_alias(value)
...
class TestConstantsIntegration:
    ...
    def test_type_aliases_with_validation_patterns(self) -> None:
        ...
        # Valid service name should work with ServiceName type alias
        valid_service = "test_service"
        if service_name_pattern.match(valid_service):
            service_name = ServiceName(valid_service)
            assert isinstance(service_name, str)

        # Valid identifier should work with ConfigKey/EntityId
        valid_identifier = "valid_identifier"
        if identifier_pattern.match(valid_identifier):
            config_key = ConfigKey(valid_identifier)
            entity_id = EntityId(valid_identifier)
            assert isinstance(config_key, str)
            assert isinstance(entity_id, str)
