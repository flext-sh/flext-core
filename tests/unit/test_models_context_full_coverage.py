...
_ModelWithNoCallableDump = m.Core.Tests.ModelWithNoCallableDump


class TestModelsContextFullCoverage:
    """Full coverage tests for Context models."""

    def test_to_general_value_dict_removed(self) -> None:
        """to_general_value_dict was removed during infra migration."""
        tm.that(not hasattr(m, "to_general_value_dict"), eq=True)

    def test_structlog_proxy_context_var_get_set_reset_paths(self) -> None:
    ...
    def test_context_data_normalize_and_json_checks(self) -> None:
        nested: t.RecursiveContainer = {"a": [{"b": 1}]}
        m.ContextData.normalize_to_container(nested)
        check_input: t.ValueOrModel = {"k": [1, "x"]}
        check_result = m.ContextData.check_json_serializable(check_input)
        tm.that(check_result, none=True)
        check_json = getattr(m.ContextData, "check_json_serializable")
        with pytest.raises(TypeError):
            check_json({"normalized"})
        normalize_to_container = getattr(m.ContextData, "normalize_to_container")
        with pytest.raises(TypeError):
            normalize_to_container(_ModelWithNoCallableDump())

    def test_context_data_validate_dict_serializable_error_paths(self) -> None:
        validate_dict = getattr(m.ContextData, "validate_dict_serializable")
        with pytest.raises(ValueError) as exc_info:
            _ = validate_dict("123")
        tm.that(exc_info.value, none=False)
        with pytest.raises(TypeError) as exc_info2:
            _ = validate_dict(_ModelWithNoCallableDump())
    ...
    def test_context_data_validate_dict_serializable_none_and_mapping(self) -> None:
    ...
    def test_context_export_serializable_and_validators(self) -> None:
        check_input2: t.ValueOrModel = {"k": [1, True]}
        check_result = m.ContextData.check_json_serializable(check_input2)
        tm.that(check_result, none=True)
        check_json2 = getattr(m.ContextData, "check_json_serializable")
        with pytest.raises(TypeError):
            _ = check_json2({"normalized"})
        validate_export = getattr(m.ContextExport, "validate_dict_serializable")
        with pytest.raises(TypeError):
            _ = validate_export(_ModelWithNoCallableDump())
    ...
    def test_context_export_validate_dict_serializable_mapping_and_models(self) -> None:
        ...
        tm.that(result_mapping, eq={"k": "v"})
        validate_export2 = getattr(m.ContextExport, "validate_dict_serializable")
        with pytest.raises(ValueError):
            _ = validate_export2("123")
    ...
    def test_context_export_statistics_validator_and_computed_fields(self) -> None:
        ...
        bad_val: t.ValueOrModel = "x"
        with pytest.raises(ValueError, match="Cannot normalize"):
            _ = m.normalize_to_mapping(bad_val)
    ...
    def test_scope_data_validators_and_errors(self) -> None:
        ...
        bad_int: t.ValueOrModel = 123
        with pytest.raises(ValueError, match="Cannot normalize"):
            _ = m.normalize_to_mapping(bad_int)
        ...
        bad_int2: t.ValueOrModel = 123
        with pytest.raises(ValueError, match="Cannot normalize"):
            _ = m.normalize_to_mapping(bad_int2)

    def test_statistics_and_custom_fields_validators(self) -> None:
        ...
        bad_str_val: t.ValueOrModel = "bad"
        with pytest.raises(ValueError, match="Cannot normalize"):
            _ = m.normalize_to_mapping(bad_str_val)
        ...
        result_none2 = m.normalize_to_mapping(None)
        tm.that(result_none2, eq={})
        bad_str_val2: t.ValueOrModel = "bad"
        with pytest.raises(ValueError, match="Cannot normalize"):
            _ = m.normalize_to_mapping(bad_str_val2)

    def test_context_data_metadata_normalizer_removed(self) -> None:
