...
from flext_infra.patterns import FlextInfraUtilitiesPatterns
...
class TestFlextInfraPatternsTooling:
    ...
    def test_mypy_hint_pattern_matches_valid_hint(self) -> None:
        ...
        match = FlextInfraUtilitiesPatterns.MYPY_HINT_RE.search(text)
    ...
    def test_mypy_stub_pattern_matches_missing_stubs(self) -> None:
        ...
        match = FlextInfraUtilitiesPatterns.MYPY_STUB_RE.search(text)
        assert match is not None
    ...
    def test_internal_prefixes_contains_flext(self) -> None:
        assert "flext_" in FlextInfraUtilitiesPatterns.INTERNAL_PREFIXES

    def test_internal_prefixes_is_tuple(self) -> None:
        assert isinstance(FlextInfraUtilitiesPatterns.INTERNAL_PREFIXES, tuple)
...
class TestFlextInfraPatternsMarkdown:
    ...
    def test_markdown_link_pattern_multiple_matches(self) -> None:
        ...
        matches = FlextInfraUtilitiesPatterns.MARKDOWN_LINK_RE.findall(text)
    ...
    def test_markdown_link_url_pattern_captures_url_only(self) -> None:
        text = "[Click here](https://example.com)"
        match = FlextInfraUtilitiesPatterns.MARKDOWN_LINK_URL_RE.search(text)
        assert match is not None
    ...
    def test_heading_pattern_matches_h1(self) -> None:
        text = "# Main Title"
        match = FlextInfraUtilitiesPatterns.HEADING_RE.search(text)
        assert match is not None
    ...
    def test_heading_pattern_multiline(self) -> None:
        text = "# Title 1\nSome content\n## Title 2"
        matches = FlextInfraUtilitiesPatterns.HEADING_RE.findall(text)
        assert len(matches) == 2
    ...
    def test_inline_code_pattern_multiple_matches(self) -> None:
        ...
        matches = FlextInfraUtilitiesPatterns.INLINE_CODE_RE.findall(text)
        assert len(matches) == 2
