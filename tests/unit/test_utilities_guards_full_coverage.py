...
def test_is_container_negative_paths_and_callable() -> None:
    handler_input: t.GuardInput = _sample_handler  # type: ignore[assignment]
    tm.that(
        callable(_sample_handler) or u.container(handler_input),
        eq=True,
    )
    tm.that(u.container([1, "x", None]), eq=True)
    tm.that(u.container({"k": 1}), eq=True)
    set_list_input: t.GuardInput = [{"x"}]  # type: ignore[assignment]
    tm.that(not u.container(set_list_input), eq=True)
    int_key_input: t.GuardInput = {1: "x"}  # type: ignore[assignment]
    tm.that(u.container(int_key_input), eq=True)
    set_val_input: t.GuardInput = {"x": {1}}  # type: ignore[assignment]
...
def test_non_empty_and_normalize_branches() -> None:
    ...
    set_meta_input: t.RuntimeData = {1, 2}  # type: ignore[assignment]
    tm.that(u.normalize_to_metadata(set_meta_input), is_=str)


def test_configuration_mapping_and_dict_negative_branches() -> None:
    bad_value_mapping: t.ConfigMap = {"k": {1}}  # type: ignore[assignment]
    bad_value_dict: t.Dict = {"k": {1}}  # type: ignore[assignment]
...
def test_protocol_and_simple_guard_helpers() -> None:
    ...
    logger_input: t.RecursiveContainer = _LoggerLike()  # type: ignore[assignment]
    tm.that(
        not _matches_type_obj(logger_input, "logger"),
        eq=True,
    )
    result_input: t.RecursiveContainer = r[int].ok(1)  # type: ignore[assignment]
    tm.that(_matches_type_obj(result_input, "result"), eq=True)
    tm.that(not _matches_type_obj(plain_obj, "service"), eq=True)
    tm.that(not _matches_type_obj(plain_obj, "middleware"), eq=True)
    handler_guard_input: t.GuardInput = _sample_handler  # type: ignore[assignment]
    ...
    identity_input: t.RecursiveContainer = _identity  # type: ignore[assignment]
    tm.that(_matches_type_obj(identity_input, "callable"), eq=True)
    ...
    bytes_input: t.RecursiveContainer = b"a"  # type: ignore[assignment]
    ...
    model_input: t.GuardInput = m.Core.Tests._Model.model_validate({"value": 1})  # type: ignore[assignment]
    tm.that(
...
def test_is_type_non_empty_unknown_and_tuple_and_fallback() -> None:
    ...
    set_input: t.RecursiveContainer = value_set  # type: ignore[assignment]
    tm.that(
        not _matches_type_obj(set_input, "string_non_empty"),
        eq=True,
    )
    ...
    invalid_spec: str | type | tuple[type, ...] = 123  # type: ignore[assignment]
...
def test_chk_exercises_missed_branches() -> None:
    ...
    bad_is_input: t.RecursiveContainer = MutableSequence[int]  # type: ignore[assignment]
    with pytest.raises(ValidationError):
        u.chk("a", is_=bad_is_input)
    bad_not_input: t.RecursiveContainer = MutableSequence[int]  # type: ignore[assignment]
    with pytest.raises(ValidationError):
...
def test_guards_bool_shortcut_and_issubclass_typeerror(
    ...
    some_type_input: t.RecursiveContainer = _SomeType  # type: ignore[assignment]
    tm.that(
        not _matches_type_obj(some_type_input, "handler"),
        eq=True,
    )
...
def test_guards_handler_type_issubclass_typeerror_branch_direct() -> None:
    ...
    try:
        candidate_input: t.RecursiveContainer = _Candidate  # type: ignore[assignment]
        tm.that(
            not _matches_type_obj(candidate_input, "handler"),
            eq=True,
        )
    finally:
...
def test_guards_issubclass_typeerror_when_class_not_treated_as_callable(
    ...
    candidate_input2: t.RecursiveContainer = _Candidate  # type: ignore[assignment]
    tm.that(
        not _matches_type_obj(candidate_input2, "handler"),
        eq=True,
    )
...
def test_guards_issubclass_success_when_callable_is_patched(
    ...
    model_sub_input: t.RecursiveContainer = _ModelSub  # type: ignore[assignment]
    tm.that(
        _matches_type_obj(model_sub_input, "handler") is False,
        eq=True,
    )
