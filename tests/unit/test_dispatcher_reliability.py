...
class TestDispatcherReliability:
    ...
    def test_circuit_breaker_transitions_and_metrics(self) -> None:
    ...
    def test_rate_limiter_blocks_then_recovers(self) -> None:
        ...
        tm.that(
            isinstance(retry_after_val, (int, float)) and retry_after_val >= 0, eq=True
        )
        time.sleep(0.22)
        tm.ok(limiter.check_rate_limit(msg_type))
        limiter.cleanup()
    ...
    def test_circuit_breaker_half_open_and_rate_limiter_public_contract(self) -> None:
        ...
        rl = FlextModelsDispatcher.RateLimiterManager(
            max_requests=1, window_seconds=1.5
        )
        assert rl.max_requests == 1
        assert abs(rl.window_seconds - 1.5) < 1e-9


__all__: list[str] = ["TestDispatcherReliability"]
