...
def test_circuit_breaker_transitions_and_metrics() -> None:
    ...
    tm.fail(failure)
    tm.that(failure.error_code, eq=c.Errors.OPERATION_ERROR)
    tm.that(cb.is_open(message_type), eq=True)
    time.sleep(0.12)
    half_open = cb.check_before_dispatch(message_type)
    tm.ok(half_open)
    tm.that(cb.get_state(message_type), eq=c.Reliability.CircuitBreakerState.HALF_OPEN)
    cb.record_success(message_type)
    tm.that(cb.get_state(message_type), eq=c.Reliability.CircuitBreakerState.CLOSED)
