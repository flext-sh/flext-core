"""FlextValidations comprehensive tests with corrected API usage.
...
class TestFlextValidationsComprehensive:
    """Comprehensive FlextValidations tests with corrected API usage targeting 33 uncovered lines."""
    ...
    def test_validate_non_empty_string_func_success(self) -> None:
        """Test validate_non_empty_string function returns boolean success."""
        result = FlextValidations.validate_non_empty_string("test")
        assert result is True

    def test_validate_non_empty_string_func_failure(self) -> None:
        """Test validate_non_empty_string function returns boolean failure."""
        result = FlextValidations.validate_non_empty_string("")
        assert result is False
    ...
    def test_predicates_class_call_failure(self) -> None:
        ...
        predicates = FlextValidations.Predicates()
        result = predicates(None)  # Assume None fails validation
        FlextTestsMatchers.assert_result_failure(result)

    def test_predicates_class_call_exception(self) -> None:
        """Test Predicates class __call__ with exception handling."""

        # Create a predicate that raises an exception
        def failing_predicate(_value: object) -> Never:
            msg = "Test error"
            raise ValueError(msg)

        predicates = FlextValidations.Predicates(failing_predicate, "failing_predicate")
    ...
    def test_supports_int_protocol_implementation(self) -> None:
        ...
        # Create a class that implements the SupportsInt protocol
        class TestIntObject:
            def __int__(self) -> int:
                return 42

        supports_int_instance = TestIntObject()
        # Test that it has __int__ method
        assert hasattr(supports_int_instance, "__int__")
        # Test that it can be converted to int
        int_value = int(supports_int_instance)
    ...
    def test_supports_float_protocol_implementation(self) -> None:
        ...
        # Create a class that implements the SupportsFloat protocol
        class TestFloatObject:
            def __float__(self) -> float:
                return math.pi

        supports_float_instance = TestFloatObject()
        # Test that it has __float__ method
        assert hasattr(supports_float_instance, "__float__")
        # Test that it can be converted to float
        float_value = float(supports_float_instance)
        assert isinstance(float_value, float)
        assert float_value == math.pi

    def test_constants_and_aliases_exist(self) -> None:
        """Test that constants and aliases are properly defined."""
        # Test core namespace aliases
    ...
    def test_core_namespace_aliases(self) -> None:
        """Test core namespace aliases functionality."""
        # Test that Core namespace provides access to main validators
        assert hasattr(FlextValidations.Core, "TypeValidators")
        assert hasattr(FlextValidations.Core, "FieldValidators")
        assert hasattr(FlextValidations.Core, "BusinessValidators")
