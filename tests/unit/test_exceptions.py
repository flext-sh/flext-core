...
class TestExceptions100PercentCoverage:
    ...
    def test_permission_error_with_required_permission(self) -> None:
        ...
        error = FlextExceptions.PermissionError(
            "Permission denied", required_permission="REDACTED_LDAP_BIND_PASSWORD_access"
        )

        assert isinstance(error, FlextExceptions._PermissionError)
        assert error.message == "Permission denied"
        assert error.required_permission == "REDACTED_LDAP_BIND_PASSWORD_access"
        assert error.context["required_permission"] == "REDACTED_LDAP_BIND_PASSWORD_access"
