...
class TestTypingsFullCoverage:
    ...
    def test_validator_callable_invocation(self) -> None:
        ...
        def upper(v: t.ScalarOrModel | None) -> t.ScalarOrModel | None:
    ...
    def test_items(self) -> None:
        """items() returns mapping items for config containers."""
        config = m.ConfigMap(root={"field1": "v1"})
        items = list(config.items())
        assert len(items) == 1
        assert items[0][0] == "field1"

    def test_values(self) -> None:
        """values() returns mapping values for config containers."""
        config = m.ConfigMap(root={"field1": "v1"})
