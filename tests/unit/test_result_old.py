...
# ============================================================================
# MODERN TEST PATTERNS WITH SOLID PRINCIPLES
# ============================================================================

class TestResultModernPatterns:
    """Demonstrate modern testing patterns following SOLID principles.

    - Single Responsibility: Each test method tests one specific behavior
    - Open/Closed: Tests can be extended with new patterns without modification
    - Liskov Substitution: Test doubles can substitute for real implementations
    - Interface Segregation: Tests use minimal, focused interfaces
    - Dependency Inversion: Tests depend on abstractions via fixtures
    """

    @pytest.mark.parametrize(("success_value", "expected_type"), [
        (42, int),
        ("test", str),
        ([1, 2, 3], list),
        ({"key": "value"}, dict),
    ])
    def test_success_result_parametrized(
        self,
        success_value: object,
        expected_type: type,
    ) -> None:
        """Test success results with different value types using parametrize."""
    ...
    @pytest.mark.parametrize(("error_message", "error_code"), [
        ("Simple error", "SIMPLE"),
        ("Complex error message", "COMPLEX"),
        ("", "EMPTY"),
        ("Special chars !@#$%", "SPECIAL"),
    ])
    def test_failure_result_parametrized(
        self,
        error_message: str,
        error_code: str,
    ) -> None:
        """Test failure results with different error patterns using parametrize."""
        result = FlextResult.fail(error_message, error_code=error_code)
    ...
    def test_railway_pattern_chain_integration(
        self,
        assert_helpers: dict[str, Callable],
    ) -> None:
        """Test railway pattern with helper functions (Dependency Inversion)."""
        # Arrange - complex chaining scenario
        initial_value = 10

        # Act - railway pattern chaining
        result = (
            FlextResult.ok(initial_value)
            .map(lambda x: x * 2)  # 20
            .flat_map(lambda x: FlextResult.ok(x + 5))  # 25
            .map(lambda x: x / 5)  # 5.0
        )

        # Assert - using injected helper functions
        assert_helpers["assert_success"](result)
        assert_helpers["assert_value_equals"](result, 5.0)

    @pytest.mark.benchmark
    def test_result_performance_characteristics(self, benchmark) -> None:
        """Test FlextResult performance using pytest-benchmark."""
        def create_and_chain_results() -> FlextResult:
            return (
                FlextResult.ok(1)
                .map(lambda x: x + 1)
                .flat_map(lambda x: FlextResult.ok(x * 2))
                .map(lambda x: x ** 2)
            )
        ...
        assert result.success
        assert result.value == 16  # ((1 + 1) * 2) ** 2

    @pytest.fixture
    def complex_calculation_service(self) -> Callable[[int], FlextResult]:
        """Factory fixture providing calculation service (Factory Pattern)."""
        def calculate(value: int) -> FlextResult:
            if value < 0:
                return FlextResult.fail("Negative values not supported")
            if value > 1000:
                return FlextResult.fail("Value too large")
            return FlextResult.ok(value ** 2 + value)

        return calculate

    def test_service_integration_with_fixtures(
        self,
        complex_calculation_service: Callable[[int], FlextResult],
    ) -> None:
        """Test service integration using dependency injection via fixtures."""
        # Test successful calculation
        result = complex_calculation_service(5)
        assert result.success
        assert result.value == 30  # 5^2 + 5 = 30

        # Test validation
        negative_result = complex_calculation_service(-1)
        assert negative_result.failure
        assert "Negative values" in negative_result.error

        # Test boundary condition
        large_result = complex_calculation_service(1001)
        assert large_result.failure
...
class TestFlextResultProperties:
    ...
    def test_error_code_property_failure(self) -> None:
        ...
        result: FlextResult[None] = FlextResult[None].fail("error", error_code="E001")
    ...
    def test_error_data_property_failure_none(self) -> None:
        ...
        result: FlextResult[None] = FlextResult[None].fail("error", error_data=None)
        if result.error_data != {}:
            msg: str = f"Expected {{}}, got {result.error_data}"
            raise AssertionError(msg)
...
class TestFlextResultFlatMapExceptions:
    ...
    def test_flat_map_type_error_exception(self) -> None:
        ...
        def failing_func(_x: str) -> FlextResult[int]:
            ...
            # return FlextResult[None].ok(42)  # Never reached
    ...
    def test_flat_map_index_error_exception(self) -> None:
        ...
        result = FlextResult[list[int]].ok([1, 2, 3])

        def failing_func(x: list[int]) -> FlextResult[int]:
            return FlextResult[int].ok(x[10])  # Index out of range
    ...
    def test_flat_map_key_error_exception(self) -> None:
        ...
        result = FlextResult[dict[str, int]].ok({"a": 1})

        def failing_func(x: dict[str, int]) -> FlextResult[int]:
            return FlextResult[int].ok(x["missing_key"])
...
class TestFlextResultUnwrapOr:
    ...
    def test_unwrap_or_failure_returns_default(self) -> None:
        ...
        result: FlextResult[None] = FlextResult[None].fail("error")
        if result.unwrap_or(None) is not None:
            msg: str = f"Expected None, got {result.unwrap_or(None)}"
            raise AssertionError(msg)
...
class TestFlextResultHash:
    ...
    def test_hash_success_with_hashable_data(self) -> None:
        ...
        result1 = FlextResult[str].ok("test")
    ...
    def test_hash_success_with_unhashable_data(self) -> None:
        ...
        result1 = FlextResult[dict[str, str]].ok({"key": "value"})
        result2 = FlextResult[dict[str, str]].ok({"key": "value"})
    ...
    def test_hash_failure_results(self) -> None:
        ...
        result1 = FlextResult[object].fail("error", error_code="E001")
        result2 = FlextResult[object].fail("error", error_code="E001")
        if hash(result1) != hash(result2):
            msg: str = f"Expected {hash(result2)}, got {hash(result1)}"
            raise AssertionError(msg)
...
class TestFlextResultRailwayMethods:
    ...
    def test_or_else_success(self) -> None:
        ...
        alternative = FlextResult[None].ok(None)
    ...
    def test_or_else_failure(self) -> None:
        ...
        if chosen.data is not None:
            msg: str = f"Expected None, got {chosen.data}"
            raise AssertionError(msg)
    ...
    def test_or_else_get_failure(self) -> None:
        ...
        def get_alternative() -> FlextResult[None]:
    ...
    def test_or_else_get_with_exception(self) -> None:
        ...
        def failing_func() -> FlextResult[None]:
            msg = "Function failed"
    ...
    def test_recover_failure(self) -> None:
        ...
        def recovery(_error: str) -> None:
            # Recovery for FlextResult[None] should return None
            return None

        recovered = result.recover(recovery)
        assert recovered.success
        if recovered.data is not None:
            msg: str = f"Expected None, got {recovered.data}"
            raise AssertionError(msg)
    ...
    def test_recover_with_exception(self) -> None:
        ...
        def failing_recovery(_error: str) -> None:
            msg = "Recovery failed"
            raise ValueError(msg)
    ...
    def test_recover_with_failure(self) -> None:
        ...
        def recovery(_error: str) -> FlextResult[None]:
            return FlextResult[None].ok(None)
    ...
    def test_recover_with_none_error_add(self) -> None:
        ...
        def recovery(error: str) -> FlextResult[str]:  # noqa: ARG001
            return FlextResult[str].ok("recovered")
    ...
    def test_recover_with_exception_add(self) -> None:
        ...
        def failing_recovery(_error: str) -> FlextResult[None]:
            msg = "Recovery failed"
            raise ValueError(msg)
...
class TestFlextResultSideEffects:
    ...
    def test_tap_failure(self) -> None:
        ...
        def side_effect(data: None) -> None:
            side_effect_called.append(data)

        returned = result.tap(side_effect)
        assert returned is result
        if side_effect_called:  # Should not call on failure:
            msg: str = f"Expected {[]}, got {side_effect_called}"
            raise AssertionError(msg)
    ...
    def test_tap_error_success(self) -> None:
        ...
        assert returned is result
        if side_effect_called:  # Should not call on success:
            msg: str = f"Expected {[]}, got {side_effect_called}"
            raise AssertionError(msg)

    def test_tap_error_failure(self) -> None:
        """Test tap_error method with failure result."""
        result: FlextResult[None] = FlextResult[None].fail("test_error")
        side_effect_called = []
    ...
    def test_tap_error_with_none_error(self) -> None:
        ...
        returned = result.tap_error(error_side_effect)
        assert returned is result
        if side_effect_called:  # Should not call with None error:
            msg: str = f"Expected {[]}, got {side_effect_called}"
            raise AssertionError(msg)
...
class TestFlextResultFilter:
    ...
    def test_filter_failure(self) -> None:
        ...
        result: FlextResult[int] = FlextResult[None].fail("initial error")
...
class TestFlextResultZipWith:
    ...
    def test_zip_with_first_failure(self) -> None:
        ...
        result1: FlextResult[int] = FlextResult[None].fail("first error")
        result2 = FlextResult[int].ok(10)
    ...
    def test_zip_with_second_failure(self) -> None:
        ...
        result1 = FlextResult[int].ok(5)
        result2: FlextResult[int] = FlextResult[None].fail("second error")
    ...
    def test_zip_with_first_none_data(self) -> None:
        ...
        result1 = FlextResult[None].ok(None)
        result2 = FlextResult[int].ok(10)
    ...
    def test_zip_with_second_none_data(self) -> None:
        ...
        result1 = FlextResult[int].ok(5)
        result2 = FlextResult[None].ok(None)
...
class TestFlextResultConversion:
    ...
    def test_to_exception_failure(self) -> None:
        ...
        result: FlextResult[None] = FlextResult[None].fail("error message")
        exception = result.to_exception()
...
class TestFlextResultStaticMethods:
    ...
    def test_any_success_true(self) -> None:
        ...
        results: list[FlextResult[object]] = [
            FlextResult[None].fail("error1"),
            FlextResult[str].ok("success"),
            FlextResult[None].fail("error2"),
        ]
    ...
    def test_first_success_found(self) -> None:
        ...
        results: list[FlextResult[object]] = [
            FlextResult[None].fail("error1"),
            FlextResult[str].ok("first_success"),
    ...
    def test_first_success_not_found(self) -> None:
        ...
        results: list[FlextResult[object]] = [
            FlextResult[None].fail("error1"),
            FlextResult[None].fail("error2"),
            FlextResult[None].fail("last_error"),
        ]
...
class TestModuleLevelFunctions:
    ...
    def test_chain_with_failure(self) -> None:
        ...
        results: list[FlextResult[object]] = [
            FlextResult[str].ok("a"),
            FlextResult[None].fail("chain error"),
            FlextResult[str].ok("c"),
        ]
...
class TestFlextResultPropertyBased:
    ...
    def test_result_fail_preserves_error(self, error_msg: str) -> None:
        """Property: FlextResult[object].fail preserves non-empty error message."""
    ...
    def test_result_map_composition(
        ...
        result: FlextResult[int] = FlextResult[None].ok(value)
...
class TestFlextResultPerformance:
    ...
    def test_result_creation_performance(
        ...
        def create_results() -> list[FlextResult[str]]:
            return [FlextResult[str].ok(f"value_{i}") for i in range(1000)]
...
class TestFlextResultIntegration:
    ...
    def test_complex_data_pipeline(self) -> None:
        ...
        def validate_data(data: object) -> FlextResult[dict[str, object]]:
            if not isinstance(data, dict):
                return FlextResult[None].fail("Data must be dictionary")
            if "id" not in data:
                return FlextResult[None].fail("Missing required 'id' field")
    ...
    def test_error_propagation_in_pipeline(self) -> None:
        ...
        def failing_step(_data: object) -> FlextResult[object]:
            return FlextResult[None].fail("Processing failed at step 2")

        def later_step(data: object) -> FlextResult[object]:
            return FlextResult[str].ok(f"processed: {data}")
...
class TestFlextResultBoundary:
    ...
    def test_deep_nesting_chain(self) -> None:
        ...
        result: FlextResult[int] = FlextResult[None].ok(1)
    ...
    def test_large_data_handling(self) -> None:
        ...
        result = FlextResult[None].ok(large_list)
...
class TestFlextResultAsync:
    ...
    async def test_result_in_async_context(self) -> None:
        ...
        async def async_operation(value: int) -> FlextResult[int]:
            ...
            return FlextResult[None].ok(value * 2)
