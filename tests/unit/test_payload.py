"""Comprehensive tests for FlextModels.Payload system.
...
class TestFlextPayload:
    """Test FlextModels.Payload core functionality with advanced pytest patterns."""
...
class TestFlextEventCoverage:
    ...
    def test_create_event_validation_error(self) -> None:
        ...
        if result.success:
            assert isinstance(result.value, FlextModels.Payload)
        else:
    ...
    def test_event_property_version_invalid(self) -> None:
        ...
        result = FlextModels.Payload.create_event("TestEvent", {"data": "test"})
        if result.success:
            event = result.value
...
class TestFlextMessageCoverage:
    ...
    def test_create_message_none(self) -> None:
        ...
        result = FlextModels.Payload.create_message("")  # Use empty string for type safety
        assert result.is_failure
        assert "Message cannot be empty" in (result.error or "")

    def test_create_message_invalid_level(self) -> None:
        """Test create_message with invalid level (lines 659-661)."""
        result = FlextModels.Payload.create_message("Test message", level="invalid_level")
    ...
    def test_create_message_validation_error(self) -> None:
        ...
        result = FlextModels.Payload.create_message("Valid message")
...
class TestFlextPayloadCoverage:
    """Test FlextModels.Payload for covering missing lines - DRY REFACTORED."""
    ...
    def test_from_dict_invalid_metadata(self) -> None:
        ...
        result: FlextResult[FlextModels.Payload[object]] = FlextModels.Payload.from_dict(invalid_data)
    ...
    def test_to_dict_basic_mixin_attributes_skip(self) -> None:
        ...
        payload = FlextModels.Payload(data="test")
    ...
    def test_serialization_collection_handling(self) -> None:
        ...
        payload = FlextModels.Payload(data=collection)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ["__str__", "__repr__"],
            ["to_dict_basic"] if hasattr(FlextModels.Payload, "to_dict_basic") else [],
        ],
    )
    def test_payload_operations_comprehensive(
        ...
        for data in basic_data[:3]:  # Limit for performance
            payload = FlextModels.Payload(data=data)
