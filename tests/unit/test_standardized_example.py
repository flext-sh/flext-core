...
class TestStandardizedExample:
    ...
    def test_config_with_fixtures(self) -> None:
        ...
        # Test config creation using direct keyword overrides (returns FlextConfig)
        config = FlextConfig.create(app_name="fixture_test")
        assert isinstance(config, FlextConfig)
        assert config.app_name == "fixture_test"

        # Pydantic-based validation round-trip to ensure the config stays valid
        reloaded_config = FlextConfig.model_validate(config.model_dump())
