...
class TestReadDoc:
    def test_read_doc_valid_file(self, tmp_path: Path) -> None:
        ...
        h.assert_file_exists(toml_file)
...
class TestWorkspaceRoot:
    def test_workspace_root_with_gitmodules(self, tmp_path: Path) -> None:
        ...
        tm.that(_workspace_root(tmp_path / "subdir"), eq=tmp_path)
    ...
    def test_workspace_root_fallback(self, tmp_path: Path) -> None:
        ...
        tm.that(_workspace_root(deep_path) is not None, eq=True)


class TestParser:
    def test_parser_args(self) -> None:
        parser = _parser()
        tm.that(parser is not None, eq=True)
