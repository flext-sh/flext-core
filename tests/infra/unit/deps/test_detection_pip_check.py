...
class TestRunPipCheck:
    ...
    def test_success_with_output(self, tmp_path: Path, monkeypatch) -> None:
        service = FlextInfraDependencyDetectionService()
        venv_bin = tmp_path / "venv" / "bin"
        venv_bin.mkdir(parents=True)
    ...
    def test_runner_failure(self, tmp_path: Path, monkeypatch) -> None:
        ...
        (venv_bin / "pip").write_text("")
        monkeypatch.setattr(
            service, "runner", _StubRunner(r[SimpleNamespace].fail("pip failed"))
        )
        tm.fail(service.run_pip_check(tmp_path, venv_bin))

    def test_success_no_issues(self, tmp_path: Path, monkeypatch) -> None:
        service = FlextInfraDependencyDetectionService()
        venv_bin = tmp_path / "venv" / "bin"
        venv_bin.mkdir(parents=True)
        (venv_bin / "pip").write_text("")
