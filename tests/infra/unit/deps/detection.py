...
class TestToInfraValue:
    ...
    def test_list_with_unconvertible(self) -> None:
        ...
        result = _to_infra_value([Path("/tmp")])
    ...
    def test_mapping_with_unconvertible(self) -> None:
        ...
        result = _to_infra_value({"key": Path("/tmp")})
        assert result is None
...
class TestLoadDependencyLimits:
    ...
    def test_success(self) -> None:
        ...
        service = FlextInfraDependencyDetectionService()
        service.toml = Mock()
        service.toml.read_plain.return_value = r[dict[str, t.Any]].ok({
            "key": "value",
            "num": 42,
        })
    ...
    def test_failure_returns_empty(self) -> None:
        ...
        service = FlextInfraDependencyDetectionService()
        service.toml = Mock()
        service.toml.read_plain.return_value = r[dict[str, t.Any]].fail("not found")
        result = service.load_dependency_limits(Path("/fake/limits.toml"))
        assert result == {}
    ...
    def test_none_value_preserved(self) -> None:
        ...
        service = FlextInfraDependencyDetectionService()
        service.toml = Mock()
        service.toml.read_plain.return_value = r[dict[str, t.Any]].ok({"key": None})
        result = service.load_dependency_limits(Path("/fake/limits.toml"))
