...
class TestRewritePoetry:
    def test_rewrite_poetry_no_tool(self) -> None:
        tm.that(FlextInfraDependencyPathSync._rewrite_poetry(TOMLDocument(), is_root=True, mode="workspace"), eq=[])

    def test_rewrite_poetry_no_poetry(self) -> None:
        doc = TOMLDocument()
        doc["tool"] = tomlkit.table()
        tm.that(FlextInfraDependencyPathSync._rewrite_poetry(doc, is_root=True, mode="workspace"), eq=[])
    ...
    def test_rewrite_poetry_rewrite_path_dep(self) -> None:
        ...
        changes = FlextInfraDependencyPathSync._rewrite_poetry(doc, is_root=True, mode="workspace")
    ...
    def test_rewrite_poetry_subproject_mode(self) -> None:
        ...
        changes = FlextInfraDependencyPathSync._rewrite_poetry(doc, is_root=False, mode="workspace")
        tm.that(len(changes) > 0, eq=True)
        tm.that(doc.as_string(), contains='path = "../flext-core"')
...
def test_rewrite_poetry_with_non_dict_value() -> None:
    ...
    tool["poetry"] = poetry
    doc["tool"] = tool
    tm.that(len(FlextInfraDependencyPathSync._rewrite_poetry(doc, is_root=False, mode="workspace")), eq=0)


def test_rewrite_poetry_no_tool_table() -> None:
    tm.that(
        len(FlextInfraDependencyPathSync._rewrite_poetry(tomlkit.document(), is_root=False, mode="workspace")), eq=0
    )
