...
class TestParseRepoMap:
    ...
    def test_parse_repo_map_success(self) -> None:
        ...
        service = FlextInfraInternalDependencySyncService()
        service.toml = Mock()
        service.toml.read_plain.return_value = r[dict[str, object]].ok({
            "repo": {
                "flext-core": {
    ...
    def test_parse_repo_map_read_failure(self) -> None:
        ...
        service = FlextInfraInternalDependencySyncService()
        service.toml = Mock()
        service.toml.read_plain.return_value = r[dict[str, object]].fail(
            "file not found"
        )
        result = service.parse_repo_map(Path("/fake/map.toml"))
        assert result.is_failure
...
class TestCollectInternalDeps:
    ...
    def test_read_failure(self, tmp_path: Path) -> None:
        ...
        service = FlextInfraInternalDependencySyncService()
        service.toml = Mock()
        service.toml.read_plain.return_value = r[dict[str, object]].fail("parse error")
        (tmp_path / "pyproject.toml").write_text("")
        result = service.collect_internal_deps(tmp_path)
        assert result.is_failure
...
class TestSync:
    ...
    def test_sync_collect_failure(self, tmp_path: Path) -> None:
        ...
        service = FlextInfraInternalDependencySyncService()
        service.toml = Mock()
        service.toml.read_plain.return_value = r[dict[str, object]].fail("read error")
        (tmp_path / "pyproject.toml").write_text("")
        result = service.sync(tmp_path)
        assert result.is_failure
    ...
    def test_sync_missing_repo_mapping(self, tmp_path: Path) -> None:
        ...
        service = FlextInfraInternalDependencySyncService()
        service.toml = Mock()
        service.toml.read_plain.side_effect = [
