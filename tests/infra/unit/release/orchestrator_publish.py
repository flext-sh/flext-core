...
def _stub_publish(mp: MonkeyPatch, root: Path) -> None:
    ...
    def _generate_notes(*a: t.ContainerValue, **kw: t.ContainerValue) -> r[bool]:
        del a, kw
...
def _stub_full_publish(mp: MonkeyPatch, root: Path) -> None:
    ...
    def _update_changelog(*a: t.ContainerValue, **kw: t.ContainerValue) -> r[bool]:
        del a, kw
        return r[bool].ok(True)

    def _create_tag(*a: t.ContainerValue, **kw: t.ContainerValue) -> r[bool]:
        del a, kw
        return r[bool].ok(True)
