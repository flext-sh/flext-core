...
def _stub_publish(mp: MonkeyPatch, root: Path) -> None:
    ...
    def _reporting_factory(*a, **kw: t.Scalar) -> FakeReporting:
        del a, kw
        return fake_rep

    def _generate_notes(*a, **kw: t.Scalar) -> r[bool]:
        del a, kw
...
def _stub_full_publish(mp: MonkeyPatch, root: Path) -> None:
    ...
    def _update_changelog(*a, **kw: t.Scalar) -> r[bool]:
        del a, kw
        return r[bool].ok(True)

    def _create_tag(*a, **kw: t.Scalar) -> r[bool]:
        del a, kw
        return r[bool].ok(True)
