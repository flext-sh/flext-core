"""Process-local registry of beartype-captured ViolationReports.
...
class FlextUtilitiesRuntimeViolationRegistry:
    """Thread-safe process-local buffer of ``m.ViolationReport`` instances."""

    _violation_buffer: ClassVar[list[_me.ViolationReport]] = []
    _violation_lock: ClassVar[threading.Lock] = threading.Lock()

    @classmethod
    def append_violation_report(cls, report: _me.ViolationReport) -> None:
        """Buffer ``report`` for later drainage by the dispatcher."""
        with cls._violation_lock:
            cls._violation_buffer.append(report)

    @classmethod
    def drain_violation_reports(cls) -> tuple[_me.ViolationReport, ...]:
