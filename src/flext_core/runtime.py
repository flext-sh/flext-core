...
class FlextRuntime:
    ...
    **Design Principles**:
    ...
    def level_based_context_filter(
        _logger: p.Log.StructlogLogger | None,
        method_name: str,
    ...
    class RuntimeResult[T](BaseModel):
        ...
        def values(self) -> list[T]:
            """Return success value as iterable list (empty on failure).

            Ergonomic pattern for iteration without cast or type narrowing.

            Example::

                for py_file in iter_result.values():
                    process(py_file)  # py_file is typed as Path

                # Equivalent to:
                if result.is_success:
                    files = cast("list[Path]", result.value)
                    for f in files: ...
        ...
        def expect_value[V](self, expected: type[V], msg: str = "") -> V:
            """Extract value with runtime type check — no cast needed.

            Combines success assertion + isinstance check + type narrowing
            in a single ergonomic call.

            Args:
                expected: Expected type of the value.
                msg: Optional custom error message prefix.

            Returns:
                Value narrowed to the expected type.

            Raises:
                RuntimeError: If result is failure or value doesn't match type.

            Example::

                config = result.expect_value(ConfigMap)
                user = service.execute().expect_value(User)
                files = iter_result.expect_value(list)

            """
            if not self.is_success:
                error_prefix = f"{msg}: " if msg else ""
                error_detail = self.error or "unknown error"
                raise RuntimeError(f"{error_prefix}Expected {expected.__name__}, got failure: {error_detail}")
            payload = self._payload
            if not isinstance(payload, expected):
                actual = type(payload).__name__ if payload is not None else "None"
                error_prefix = f"{msg}: " if msg else ""
                raise RuntimeError(f"{error_prefix}Expected {expected.__name__}, got {actual}")
            return payload

        def value_or[D](self, default: D) -> T | D:
            """Return success value or typed default (generic over default type).

            Unlike ``unwrap_or`` which constrains default to T,
            this method allows any default type D and returns T | D.

            Example::

                name: str = result.value_or("unknown")
                count: int = result.value_or(0)
