...
This module demonstrates how to use Pydantic TypeAdapter to simplify validation,
eliminate legacy BaseModel patterns, and provide type-safe serialization
throughout the FLEXT ecosystem.

Key Benefits of TypeAdapter:
- Direct type validation without BaseModel overhead
...
Usage Examples:
    ...
    email = email_adapter.validate_python("user@example.com")

    # JSON validation
    user_adapter = TypeAdapter(User)
    user = user_adapter.validate_json('{"name": "John", "email": "john@example.com"}')

    # Serialization
    json_data = user_adapter.dump_json(user)
    python_data = user_adapter.dump_python(user)
