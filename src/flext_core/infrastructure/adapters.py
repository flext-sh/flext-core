...
if TYPE_CHECKING:
    ...
    from flext_core.domain.advanced_types import MetadataDict, QueryParameters
    ...
    async def subscribe_to_events(
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                # Group likely already exists if this fails
    ...
    async def _send_email(
        ...
        try:
            email_data = {
                "to": recipient,
                "subject": (
                    metadata.get("subject", "FLEXT Platform Notification")
                    if metadata
                    else "FLEXT Platform Notification"
                ),
                "body": message,
                "from": "noreply@flext-platform.com",
            }
