...
the FLEXT platform, supporting both in-memory and distributed event processing.
"""
...
if TYPE_CHECKING:
    ...
    from flext_core.domain.advanced_types import DomainEventData, MetadataDict
...
class HybridEventBus:
    ...
    def __init__(self) -> None:
        ...
        self._connection: AbstractRobustConnection | None = None  # type: ignore[no-any-unimported]
    ...
    async def publish(
        ...
        with all FLEXT components and legacy patterns.
        ...
        if isinstance(event, str):
            ...
            event_obj = DomainEvent.create(str(event_type), event_data)
        else:
...
class InMemoryEventBus:
    ...
    async def publish(self, event: Event) -> None:
        ...
        self.logger.debug("Publishing event", event_type=event_type)
    ...
    async def _on_amqp_message(self, message: AbstractIncomingMessage) -> None:
        ...
        async with message.process():
            try:
                ...
                event = DomainEvent.create(data.get("event_type", "unknown"), data)
