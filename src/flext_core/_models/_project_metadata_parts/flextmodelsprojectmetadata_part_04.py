"""Pydantic v2 project-metadata SSOT models (flat on ``m.*``).

Tier 3 domain models owned by flext-core. Consumed across the monorepo
as ``m.ProjectMetadata`` / ``m.ProjectNamespaceConfig`` /
``m.ProjectToolFlext`` / ``m.ProjectToolFlextProject`` / etc. Immutable
by construction (``frozen=True``, ``extra="forbid"``) so callers cannot
drift metadata after load.

Architecture: Tier 3 — imports Tier 0 (_constants/pydantic) only.

Copyright (c) 2025 FLEXT Team. All rights reserved.
SPDX-License-Identifier: MIT
"""

from __future__ import annotations

from typing import Annotated, ClassVar

from pydantic import (
    Field,
)

from flext_core._models.pydantic import FlextModelsPydantic

from .flextmodelsprojectmetadata_part_03 import (
    FlextModelsProjectMetadata as FlextModelsProjectMetadataPart03,
)


class FlextModelsProjectMetadata(FlextModelsProjectMetadataPart03):
    class ProjectToolFlext(FlextModelsPydantic.BaseModel):
        """``[tool.flext]`` root contract aggregating the sub-tables."""

        model_config: ClassVar[FlextModelsPydantic.ConfigDict] = (
            FlextModelsPydantic.ConfigDict(frozen=True, extra="forbid")
        )

        project: Annotated[
            FlextModelsProjectMetadataPart03.ProjectToolFlextProject,
            Field(
                default_factory=FlextModelsProjectMetadataPart03.ProjectToolFlextProject,
                description="[tool.flext.project] sub-table.",
            ),
        ] = Field(
            default_factory=FlextModelsProjectMetadataPart03.ProjectToolFlextProject,
        )
        namespace: Annotated[
            FlextModelsProjectMetadataPart03.ProjectToolFlextNamespace,
            Field(
                default_factory=FlextModelsProjectMetadataPart03.ProjectToolFlextNamespace,
                description="[tool.flext.namespace] sub-table.",
            ),
        ] = Field(
            default_factory=FlextModelsProjectMetadataPart03.ProjectToolFlextNamespace,
        )
        docs: Annotated[
            FlextModelsProjectMetadataPart03.ProjectToolFlextDocs,
            Field(
                default_factory=FlextModelsProjectMetadataPart03.ProjectToolFlextDocs,
                description="[tool.flext.docs] sub-table.",
            ),
        ] = Field(
            default_factory=FlextModelsProjectMetadataPart03.ProjectToolFlextDocs,
        )
        aliases: Annotated[
            FlextModelsProjectMetadataPart03.ProjectToolFlextAliases,
            Field(
                default_factory=FlextModelsProjectMetadataPart03.ProjectToolFlextAliases,
                description="[tool.flext.aliases] sub-table.",
            ),
        ] = Field(
            default_factory=FlextModelsProjectMetadataPart03.ProjectToolFlextAliases,
        )
        workspace: Annotated[
            FlextModelsProjectMetadataPart03.ProjectToolFlextWorkspace,
            Field(
                default_factory=FlextModelsProjectMetadataPart03.ProjectToolFlextWorkspace,
                description="[tool.flext.workspace] sub-table.",
            ),
        ] = Field(
            default_factory=FlextModelsProjectMetadataPart03.ProjectToolFlextWorkspace,
        )


__all__: list[str] = ["FlextModelsProjectMetadata"]
