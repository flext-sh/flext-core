...
class UniversalCommandProcessor:
    ...
    def register_handler(self, handler: CommandHandler) -> None:
        ...
        for command_type in supported_commands:
            if command_type in self.handlers:
                logger.warning(
                    "Overriding existing handler for command type",
                    extra={"command_type": command_type},
                )
            self.handlers[command_type] = handler
        logger.info(
            "Registered handler for commands",
    ...
    def unregister_handler(self, command_type: str) -> None:
        ...
        if command_type in self.handlers:
            ...
            logger.info(
                "Unregistered handler for command type",
                extra={"command_type": command_type},
            )
        else:
            logger.warning(
    ...
    async def execute_command(
        ...
        try:
            ...
            logger.exception(
                ...
                extra={"command_id": command_id, "error": str(e)},
            )
            return self._create_error_result(
                command_id,
                "COMMAND_EXECUTION_ERROR",
    ...
    async def cancel_command(self, command_id: CommandID) -> bool:
        ...
        if command_id in self.active_commands:
            ...
            logger.info("Cancelling command", extra={"command_id": command_id})
            self.active_commands.pop(command_id, None)
            return True
        return False
...
class SystemCommand(BaseCommand):
    ...
    def validate(self) -> CommandValidationResult:
        ...
        # Add required permissions - system commands require admin access
        required_permissions.append("system:admin")
...
class PipelineCommandHandler(BaseCommandHandler):
    ...
    async def _create_pipeline(
        ...
        self.logger.info("Pipeline created", extra={"pipeline_id": pipeline_id})
    ...
    async def _execute_pipeline(
        ...
        self.logger.info(
            ...
            extra={"pipeline_id": pipeline_id, "execution_id": execution_id},
        )
        return ServiceResult.ok(result)
