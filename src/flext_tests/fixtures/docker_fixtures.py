...
@pytest.fixture
def client-a_oud_container(flext_docker: FlextTestDocker) -> Generator[str]:
    """Oracle Unified Directory container fixture for client-a testing.

    This fixture provides a real Oracle Unified Directory container configured
    for client-a Telecom testing with production-compatible port mapping:
    - Base DN: dc=example,dc=com (generic test data)
    - Port: 3389 (client-a production-compatible port mapping)
    ...
    Port 3389 is mapped to container's internal 1389 for client-a compatibility.
    """
    # Start the client-a OUD test container
    start_result = flext_docker.start_container("flext-client-a-oud-test")
    if start_result.is_failure:
        pytest.skip(f"client-a OUD container failed to start: {start_result.error}")

    # Get container status to extract connection info
    status_result = flext_docker.get_container_status("flext-client-a-oud-test")
    if status_result.is_failure:
        pytest.skip(f"Failed to get client-a OUD container status: {status_result.error}")

    container_info = status_result.value
    if "3389" not in container_info.ports:
        pytest.skip("client-a OUD container port 3389 not available")

    host_port = container_info.ports["3389"]

    # Return connection string for client-a tests
    connection_string = f"ldap://{FlextConstants.Platform.DEFAULT_HOST}:{host_port}"
