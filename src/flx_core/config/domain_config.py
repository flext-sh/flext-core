"""Unified configuration management for FLX Meltano Enterprise.
...
from flx_core.utils.import_fallback_patterns import SQLALCHEMY_DEPENDENCY
...
class NotificationConfiguration(BaseModel):
    ...
    notification_email: str | None = Field(
        default_factory=lambda: os.environ.get("FLX_NOTIFICATION_EMAIL", "REDACTED_LDAP_BIND_PASSWORD@flx-enterprise.com"),
...
class FlxDomainConfiguration(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_production_requirements(self) -> FlxDomainConfiguration:
...
class FlxSecretConfiguration(BaseModel):
...
class FlxConfiguration(BaseSettings):
    """FLX Configuration - Complete DDD + Pydantic + Python 3.13 Integration.
    ...
    @model_validator(mode="after")
    def apply_environment_overrides_after_creation(self) -> FlxConfiguration:
    ...
    @model_validator(mode="after")
    def validate_production_requirements(self) -> FlxConfiguration:
        """Validate production requirements."""
...
_config_cache: FlxConfiguration | None = None


def get_config() -> FlxConfiguration:
    """Get application configuration instance - SINGLE SOURCE OF TRUTH."""
    global _config_cache
    if _config_cache is None:
        _config_cache = FlxConfiguration()
