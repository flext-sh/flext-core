...
from flx_core.utils.import_fallback_patterns import get_redis_client

if TYPE_CHECKING:
    import redis

    from flx_core.plugins.manager import PluginManager
else:
    # Redis managed through centralized fallback patterns
    redis = (
        get_redis_client() or type("StubRedis", (), {"Redis": lambda *a, **k: None})()
    )

from flx_meltano.unified_anti_corruption_layer import UnifiedMeltanoAntiCorruptionLayer

from flx_core.commands.base import ReflectionCommandBus
...
class FlxApplicationContainer:
    """Central dependency injection container for FLX Meltano Enterprise.
    ...
    def __init__(self, settings: FlxConfiguration) -> None:
    ...
    @property
    def settings(self) -> FlxConfiguration:
    ...
    def _get_event_bus(self) -> HybridEventBus:
        ...
        if self._event_bus is None:
            ...
            event_bus_module = importlib.import_module("flx_core.events.event_bus")
    ...
    def __repr__(self) -> str:
        ...
        return f"FlxApplicationContainer(settings={self._settings.__class__.__name__})"


# Factory function for easy container creation
def create_application_container(
    settings: FlxConfiguration | None = None,
) -> FlxApplicationContainer:
    ...
    if settings is None:
        ...
        config_module = importlib.import_module("flx_core.config.domain_config")
        settings = config_module.get_config()

    return FlxApplicationContainer(settings)
