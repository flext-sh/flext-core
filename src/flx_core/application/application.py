"""FLX Enterprise Application - UNIFIED ARCHITECTURAL SUPREMACY.

ARCHITECTURAL REVOLUTION: Complete consolidation of modern_application.py with all
enterprise features, dependency injection excellence, and ZERO TOLERANCE architectural purity.
...
✅ Single Application Class - FlxEnterpriseApplication (unified architecture)
...
- FlxApplication (lato Application patterns)
...
from flx_observability.monitoring.rich_error_handler import error_context
from lato import Application, DependencyProvider

from flx_core.application.interface_bridge import InterfaceBridge
from flx_core.config.domain_config import FlxConfiguration, get_config
from flx_core.contracts.lifecycle_protocols import is_initializable, is_shutdownable

# Import for runtime use
from flx_core.infrastructure.containers import (
...
if TYPE_CHECKING:
    ...
    from flx_core.events.event_bus import DomainEventBus, HybridEventBus
...
class FlxEnterpriseApplication(Application):
    """Unified Enterprise Application with ZERO TOLERANCE architectural supremacy.

    ARCHITECTURAL EXCELLENCE:
    This application class unifies the best of ModernFlxApplication (dependency-injector)
    and FlxApplication (lato) patterns into a single, enterprise-grade solution.
    ...
    def __init__(
        self,
        *,
        name: str = "flx-enterprise-platform",
        config: ApplicationConfig | None = None,
        container: ApplicationContainer | None = None,
        dependency_provider: DependencyProvider | None = None,
    ) -> None:
        """Initialize unified FLX enterprise application with full integration.
        ...
        # Compatibility layer for legacy FlxApplication
        self._settings = self._config  # FlxConfiguration is the unified settings object
        ...
        self.logger.info(
            "FLX Enterprise Application initialized",
    ...
    async def _wire_dependencies(self) -> None:
        ...
        try:
            ...
            core_modules = [
                "flx_core.application.application",
                "flx_core.application.handlers",
                "flx_core.application.interface_bridge",
            ]
    ...
    def get_settings(self) -> ApplicationConfig:
        ...
        Returns
        -------
            The FlxConfiguration instance for the application.
...
def create_application(
    name: str = "flx-enterprise-platform", config: ApplicationConfig | None = None
) -> FlxEnterpriseApplication:
    """Create a unified FLX enterprise application instance.
    ...
    Returns:
    -------
        FlxEnterpriseApplication: Configured application instance
    ...
    logger.info(
        "FLX Enterprise Application created",
        name=name,
...
def create_testing_application(
    ...
    Returns:
    -------
        FlxEnterpriseApplication: Testing-configured application instance
...
def create_production_application(
    name: str = "flx-enterprise-production",
) -> FlxEnterpriseApplication:
    ...
    Returns:
    -------
        FlxEnterpriseApplication: Production-configured application instance
...
FlxApplication = FlxEnterpriseApplication
ModernFlxApplication = FlxEnterpriseApplication
...
__all__ = [
    "ApplicationConfig",
    "FlxApplication",  # Legacy compatibility
    "FlxEnterpriseApplication",
