...
def _get_domain_base_classes():
    """Lazy import of domain base classes to avoid circular imports."""
    from flx_core.domain.pydantic_base import DomainAggregateRoot, DomainValueObject
...
# Python 3.13 Advanced Type Aliases - ENTERPRISE DOMAIN PATTERNS WITH VALUE OBJECTS
# Using new Python 3.13 type alias syntax for compatibility
...
def value_object[T](cls: type[T]) -> type[T]:
    ...
    if not is_frozen:
        try:
            dataclass_params = cls.__dataclass_params__
            # Check if params have frozen attribute
            try:
                frozen_attr = dataclass_params.frozen
                if frozen_attr:
                    is_frozen = True
            except AttributeError:
...
def specification[T](cls: type[T]) -> type[T]:
    ...
    if "__and__" not in cls.__dict__:
        ...
        cls.__and__ = __and__  # type: ignore[method-assign,operator]
    ...
    if "__invert__" not in cls.__dict__:
        ...
        def __invert__(self: object) -> NotSpecification[object]:
            # Cast self to proper specification type
            spec_self = cast("Specification[object]", self)
            return NotSpecification(spec=spec_self)

        cls.__invert__ = __invert__  # type: ignore[method-assign,operator]
...
def is_value_object_class(cls: type) -> bool:
    ...
    try:
        ...
        try:
            return bool(cls._is_value_object)
        except AttributeError:
...
def is_specification_class(cls: type) -> bool:
    ...
    try:
        ...
        try:
            return bool(cls._is_specification)
        except AttributeError:
...
class DomainFactory[T]:
    ...
    def create(self, **kwargs: object) -> CommandResult[T]:
