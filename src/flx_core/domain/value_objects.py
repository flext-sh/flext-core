"""Domain value objects for the FLX-Meltano Enterprise Core.
...
class Duration(DomainValueObject):
    ...
    def __init__(
        ...
        **time_units: float | None,
        ...
        super().__init__(total_seconds_value=total_seconds, **data)
...
class PluginType(Enum):
    ...
    in the FLX Meltano Enterprise platform, including extractors, loaders,
...
class PluginConfiguration(DomainValueObject):
    ...
    def __init__(
        ...
        if settings is not None:
            ...
            super().__init__(settings=combined_settings, **model_data)
        else:
            # If no settings dict, treat all kwargs as configuration settings
            super().__init__(settings=dict(data))
    ...
    def validate_settings(self) -> Self:
        ...
        for field_name, field_value in self.__dict__.items():
            if field_name != "settings" and not field_name.startswith("_"):
                # Move extra fields to settings
                if (
