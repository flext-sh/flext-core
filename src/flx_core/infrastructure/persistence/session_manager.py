...
class DatabaseSessionManager:
    ...
    def _get_database_url(self) -> str:
        ...
        if self._config.environment == "development":
            return "sqlite+aiosqlite:///./flx_enterprise.db"

        # Production PostgreSQL configuration
        db_config = getattr(self._config, "database", {})
        host = db_config.get("host", "localhost")
        port = db_config.get("port", 5432)
        username = db_config.get("username", "flx_user")
        password = db_config.get("password", "flx_password")
        database = db_config.get("database", "flx_enterprise")
