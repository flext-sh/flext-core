...
from flx_core.mappers.entity_mappers import (  # Inverse mappers for entity to model conversion
...
class DomainSpecificRepository[TEntity: BaseModel, TModel, TId](
    ...
    async def find_scheduled_pipelines(self) -> list[TEntity]:
        ...
        try:
            self.model_class.schedule_expression
            self.model_class.is_active
        except AttributeError:
            return []
        ...
        try:
            self.model_class.max_concurrent_executions
            # Additional logic could check for running executions here
            # For now, include all matching pipelines
        except AttributeError:
            # Model doesn't have max_concurrent_executions - continue
            pass
    ...
    async def get_next_execution_number(self, pipeline_id: object) -> int:
        ...
        try:
            self.model_class.execution_number
            self.model_class.pipeline_id
        except AttributeError:
            return 1
