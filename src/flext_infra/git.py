...
class FlextInfraGitService(s[str]):
    ...
    def smart_checkout(self, repo_root: Path, branch: str) -> r[bool]:
        """Checkout a branch with multiple fallback strategies.

        Tries, in order:
        1. No-op if already on *branch*
        2. Simple ``git checkout``
        3. Force-create (``-B``) if local changes conflict
        4. Fetch + checkout tracking branch
        5. Force-create as last resort
        ...
        current_result = self.current_branch(repo_root)
        if current_result.is_success and current_result.value == branch:
            return r[bool].ok(True)

        checkout_result = self.checkout(repo_root, branch)
        ...
        if "local changes" in detail or "would be overwritten" in detail:
            return self.checkout(repo_root, branch, create=True)

        fetch_result = self.fetch(repo_root, c.Infra.Git.ORIGIN, branch)
    ...
    def checkpoint(
        ...
        branch: str,
        *,
        message: str = "chore: checkpoint pending changes",
    ) -> r[bool]:
        """Add, commit and push pending changes in one call.

        Handles the full add → check staged → commit → push → rebase cycle.
        ...
        Returns:
            r[bool] with True on success (including "nothing to commit").
        ...
        add_result = self.add(repo_root)
        if add_result.is_failure:
            return r[bool].fail(add_result.error or "git add failed")

        staged_result = self.diff_names(repo_root, cached=True)
        if staged_result.is_success and not staged_result.value.strip():
            return r[bool].ok(True)

        commit_result = self.commit(repo_root, message)
    ...
    def create_tag_if_missing(
        self,
        repo_root: Path,
        tag: str,
        message: str = "",
    ) -> r[bool]:
        """Create an annotated tag only if it doesn't already exist.

        Returns:
            r[bool] with True on success (created or already exists).

        """
        exists_result = self.tag_exists(repo_root, tag)
        if exists_result.is_success and exists_result.value:
            return r[bool].ok(True)
        return self.create_tag(repo_root, tag, message or f"release: {tag}")
    ...
    def collect_changes(
        ...
        """Collect formatted Git commit messages between two tags.

        Handles the common release-notes pattern: check if *tag* exists,
        fall back to HEAD, and build the revision range.

        Args:
            repo_root: Repository root directory.
            tag: Target tag.
            previous: Previous tag (if empty, uses *target* only).

        Returns:
            r[str] with formatted commit log.

        """
        tag_result = self.tag_exists(repo_root, tag)
        target = (
            tag if (tag_result.is_success and tag_result.value) else c.Infra.Git.HEAD
        )
        rev = f"{previous}..{target}" if previous else target
        return self.log(repo_root, rev)
